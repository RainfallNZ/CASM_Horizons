...
#'
#' \code{ScenarioLeachRateRaster} prepares a leach rate raster based on an existing leach rate raster and various optional parmaters.
#'
...
#***********************************************************
#Function to find the leach rate for a vector of WMSZ, LUC, Landuse combinations
#Returns the average leach rate for all cells that match
DairyLeachRateGeterer <- function(LeachRates = LeachRate,
  ...
  RasterValue <- which(levels(ZoneLandUseLUC)[[1]]$category == Criteria)
    ...
    #average them
    AverageValue <- mean(DairyLeachRates$N_loss,na.rm=TRUE)
    return(AverageValue)
}
...
#'
#' \code{LoadComparer} returns the difference in loads for each water management sub zone of two CASM input files
#'
#' 
...
### Horizons Scenario 12a, 12b 12c

Naming system| Scenario number/name
-------------|-----------  
Horizons                 | 12[a,b,c]
Streamlined Environmental| N/A
Report                   | Post-regulation[A,B,C]  
CASM input filename      | CASM-Inputs_Horizons_Scenario12_ConsentedOrBaseOr[HorticultureInSWestTo[46,32]]CoxCalibrated.xlsx
...
6. Edit (then run) the "scenario preparation" chunk so that the "Scenario column of interest" is set to the appropriate column of the Horizons master spreadsheet. Note that this spreadsheet uses the Horizon's scenario numbering system i.e. scenario 3. 
7. Run the chunks to follow in turn until after the Diffuse Table chunk.  
8. DON'T run the Scenario 5 chunk 
9. Run the "Adjust-Diffuse-table-for-sccenarios" chunk.  
10. Edit (then run) the Excel output chunk so that the output excel file has a sensible name. 
11.Check that the output file is reasonable by running the CASMScenarioInputChecker function.  
...
### Horizons Scenario 12d

Naming system| Scenario number/name
-------------|-----------  
Horizons                 | 12d 
Streamlined Environmental| N/A
Report                   | Post-regulationD  
CASM input filename      | CASM-Inputs_Horizons_Scenario12_ConsentedOrBaseOrHorticultureInSWestTo46OrCoxCalibrated2017PointSources.xlsx

#### Scenario preparation process
1. as above
2. as above
3. as above. But also edit the PointSourceYear to be 2017
4. as above.
5. as above.
6. as above
7. as above  
...
{
##Special preparation of a table that shows the 75th percentiles of water management zones, and the 90th percentage of individual properties
#FedFarmValues <- apply(HorizonsPropertyData,1, function(x) {
#  WaterManagementZone <- x["Water.Management.Zone"]
#  WMZBaselines <- HorizonsPropertyData$`Base.N.Leaching.(kg.N/ha/yr)`[HorizonsPropertyData$Water.Management.Zone == WaterManagementZone]
#  Percentile75th <- quantile(WMZBaselines,probs = 0.75,names=FALSE,na.rm=TRUE)
#  NewLossRate <- if(x["Consented"] == "Y") as.numeric(x["Target.N.Leaching.(kg.N/ha/yr)"]) else min(Percentile75th,0.9 * as.numeric(x["Base"]),na.rm=TRUE)
#  Output <- data.frame(Percentile75th = Percentile75th,Base0.9 = 0.9 * as.numeric(x["Base"]), FedFarmRate = NewLossRate)
#  return(Output)
#})

#FedFarmTable <- do.call("rbind",FedFarmValues)
#FedFarmTable <- cbind(HorizonsPropertyData,FedFarmTable)
#FedFarmTable <- FedFarmTable[,c(1:3,9,13,15,16,61:63)]
#write.table(FedFarmTable,file.path(DataDirectory,"FedFarmScenarioDataTable.csv"),quote = FALSE,sep=",")
...
for (WaterManagementSubZone in DairyAreas$Zone_Code){
  ...
  for (LUC in LUCClasses) {
    ...
    if (nrow(DairyNodeOfInterest) < 1) {
      ...
      #browser()
      DairyNodeOfInterest$`Export Coeff (kg/ha/yr)` <- DairyLeachRateGeterer(ZoneLanduseLUCOfInterest = c(WaterManagementSubZone,"Sheep and/or Beef",LUC))
...
```{r Adjust-Diffuse-table-for-sccenarios}
...
saveWorkbook(Out, file.path(DataDirectory,"Test.xlsx"), overwrite = T)
```
