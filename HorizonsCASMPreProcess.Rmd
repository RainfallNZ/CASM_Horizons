...
ScenarioLeachRateRaster <- function(CurrentLeachRateRaster = LeachRate, LUC_To_Leach_Table = LeachRateLimits, ZoneLanduseLUCRasterFile = ZoneLanduseLUCRasterFile, ZoneLanduseLUCCode_To_ClassLUTFile = ZoneLanduseLUCCode_To_ClassLUTFile,
                                     plan = "PC2", year = 20,
                                    Intensive = FALSE,
  ...
  IrrigatedRaster <- stack(file.path(GISDataDirectory,MPIPredictorRasterFileName))[[5]]
...
#Load some auxiliary R data files
REC_MeanFlowFile  <- "REC2MeanFlow.RData"  #This is an RData file
NitrogenLoadsFile <- "N_ExpCoeff_Est_HZ_Feb20.rdata"

#Load the various shpaefiles. These are held within the GIS directory in subdirectories with the same name as the shapefile
...
#Load the various raster files
PredictorRasterFile                <- "PredictorRaster 250x250.tif"
#Baseline MPI leach rates
LeachRateRasterFile                <- "LeachRateRaster 250x250.tif"
#Various scenario leach rates
#LeachRateRasterFile                <- "ScenarioLeachRasters_IntensiveLanduseTable4-2.tif"
#LeachRateRasterFile                <- "ScenarioLeachRastersFullTable4-2.tif"

ZoneLanduseLUCRasterFile           <- "ZoneLanduseLUCRaster 250 x 250.tif"

#Load the various text files
...
ZoneLanduseLUCCode_To_ClassLUTFile <- "ZoneLanduseLUCCode_To_ClassLUT.csv"
LeachRateLimitsFileName            <- "Table14_2.csv"

#And an excel spreadsheet from Horizons
HorizonsConsentedLeachRatesFile    <- "20200417_Master dairy farm data_Working Document – For Purposes of Scenario Modelling.xlsx"
...
HorizonsPropertyData <- HorizonsPropertyData[,c(3,23:30,41:46)]
...
ScenarioColumnOfInterest <- "Consented"
...
```
Adjust the diffuse inputs for any scenario data. This is prepared earlier in a data rame called "combinedclasses".
...
saveWorkbook(Out, file.path(DataDirectory,"CASM-Inputs_ConsentedScenarioTrial.xlsx"), overwrite = T)
```
...
By Request from Ton, he would like to know the area of each MPI combination, and its N loss
```{r}
