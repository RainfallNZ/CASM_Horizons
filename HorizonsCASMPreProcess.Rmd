...
#' Function to create scenario leach rate rasters based on reginal plan leach rate limits, and optionally landuse and leach rate mitigation. 
...
### Horizons Scenario 3e-h

Naming system| Scenario number/name
-------------|-----------  
Horizons                 | 3 
Streamlined Environmental| 5 
...
3. Edit (then run) the File name setting code chunk so that the leach rate raster file name is set to Intensive Farm Land use PC2 table 14.2 tif file.  
4. Run the "load files" chunk.
5. Edit (then run) the "scenario preparation" chunk so that the "Scenario column of interest" is set to the appropriate column of the Horizons master spreadsheet. Note that this spreadsheet uses the Horizon's scenario numbering system i.e. scenario 3.  
6. Run the chunks to follow in turn until after the Diffuse Table chunk.  
7. DON'T run the "Dairy-expansion-scenario-preparation" chunk 
8. Run the "Adjust-Diffuse-table-for-sccenarios" chunk.  
9. Edit (then run) the Excel output chunk so that the output excel file has a sensible name. 
10. Check that the output file is reasonable by running the CASMScenarioInputChecker function.
...
4. Run the "load files" chunk.
5. Update the LeachRate raster using the ScenarioLeachRateRaster function with Intensive set to "TRUE", Landuse set to "Dairy" and Mitigation set to 9 (for 3i) and 18 (for 3j)
...
6. Create sheep-and-beef-adjusted LeachRate rasters using the ScenarioLeachRateRaster function with leachrate set to LeachRateCC, Intensive set to "TRUE", Landuse set to "Sheep and/or Beef" and mitigation set to 9 (for 3k) and 18 (for 3l)
7. Create hort-veg-adjusted LeachRate rasters using the ScenarioLeachRateRaster function with leachrate set to LeachRateCC, Intensive set to "TRUE", Landuse set to "Horticulture-Veg" and mitigation set to 0.36
8. Create hort-other-adjusted LeachRate rasters using the ScenarioLeachRateRaster function with leachrate set to LeachRateCC, Intensive set to "TRUE",  Landuse set to "Horticulture-Other" and mitigation set to 0.36
9. Get the maximum of the hort-veg and hort-other leach rates using max(stack(leachrateveg,leachrateother))
10. Get the maximum of the veg and the dairy and sheep and beef leach rates for each of the mitigation variants, using the same method as in 9 above
6. Edit (then run) the "scenario preparation" chunk so that the "Scenario column of interest" is set to the appropriate column of the Horizons master spreadsheet. Note that this spreadsheet uses the Horizon's scenario numbering system i.e. scenario 3.
7. Run the chunks to follow in turn until after the Diffuse Table chunk.  
8. DON'T run the "Dairy-expansion-scenario-preparation" chunk 
9. Run the "Adjust-Diffuse-table-for-sccenarios" chunk.  
...
saveWorkbook(Out, file.path(DataDirectory,"CASM-Inputs_Horizons_Scenario3j_IntensiveTable14_2Year20OrConsentedOrDH18AllDairyWithCoxCalibrated.xlsx"), overwrite = T)
```
