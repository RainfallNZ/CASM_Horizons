...
#' Leach Rate raster creator for scenarios
#'
#' ScenarioLeachRateRaster prepares a leach rate raster based on an existing leach rate raster and various optional parmaters.
#'
#' This function enables creation of new leach rate rasters that comply to Horizon's Plan Change 2 Table 14.2, 
#' or from land-use-based leach rate changes. The function uses the raster of the Sub-management Zone-Land Use-LUC, 
#' the look up table which converts the class numbers to descriptions, and the "current" MPI-based leach rate raster.
#' @param CurrentLeachRateRaster The base leach rate raster with which to modify
#' @param LUC_To_Leach_Table A csv file which has the Table 14.2 leach rates for different Land Use Capablities, and years.
#' @param ZLLRaster A raster of the zone, landue and land use capability combination codes. Same extent and resolution as CurrentLeachRateRaster
#' @param ZoneLanduseLUC_LUTFile Look up table to convert the values in the ZLLRaster to the actual zone, land uses and land use capabilities
#' @param plan Either "PC2" (the default), or "Existing".
#' @param year Which year of the Table 14.2 to apply. Default is 20. Options depend on what is in the "Year" column of the LUC_TO_LEACH_Table
#' @param Intensive Whether the leach rate ammendments are to be limited to Intensive Farm Land Uses (IFLU) or to everywhere. "FALSE" is the default.
#' @param LandUse Whether the land-use-specific ammendments are to be implemented. Options are "NA" (the default), or any of the Landuse classes in ZoneLanduseLUC_LUTFile table. "Dairy" is a popular choice. The type of ammendment is determined by the vaulue of the Mitigation parameter.
#' @param Mitigation Ignored if Landuse is "NA". The amount of reduction. If an integer then the mitigation is absolute leach rate to be reduced by (units are annual kg/ha), if a decimal less than 1, the mitigation is a relative drop in leach rate. These values are to be applied to the landuse specified by the Landuse parameter.
#' @param MPIPredictorRasterFile Raster stack of the leach rate predictors
#' @return A raster of leach rates with a side-effect of a multi-band geo tiff file of the leach rates, complete with the constituent rasters.
ScenarioLeachRateRaster <- function(CurrentLeachRateRaster = LeachRate, 
                                    LUC_To_Leach_Table = LeachRateLimits, 
                                    ZLLRaster = ZoneLanduseLUCRaster, 
                                    ZoneLanduseLUC_LUTFile = ZoneLanduseLUCCode_To_ClassLUTFile,
                                    plan = "PC2", 
                                    year = 20,
                                    Intensive = FALSE,
                                    Landuse = "NA",
  ...
  if(Landuse != "NA") {
    #Adjust the specified landuse type using the mitigation rate
    #Create a mask of the intensive land use of interest
    LandUseMask <- IntensiveFarmingMask & (LandUseRaster %in% which(levels(ZoneLanduseLUC_LUT$Landuse) %in% Landuse))
    
    #For the mask area get the original leach rate and subtract the mitigation leach rate
    MaxMitigation <- LandUseMask * CurrentLeachRateRaster - Mitigation

    #If mitigation is less than 1 assume it is supposed to be a relative mitigation reduction
    if (Mitigation < 1) MaxMitigation <- LandUseMask * CurrentLeachRateRaster * (1 - Mitigation)
    
    #For the mask area get the maximum of the Table rate or the mitigation rate. The idea is that mitigation will only be carried out until the table rate is reached.
    NewLandUseRate <- max(stack(MaxMitigation,ScenarioLeachRate))
    
    #Update the leach rate with these values
    ScenarioLeachRate <- NewLandUseRate * LandUseMask + ScenarioLeachRate * !LandUseMask
  }
...
#Function to adjust the leach rate raster in accordance with a potato-growing scenario.
...
#' Get the average "Dairy" leach rate for areas that meet a water management sub zone, landuse and landuse capability criteria.
#'
#' Calculations are done using raster maths. That is, areas from the ZoneLandUseLUC raster that meet the criteria are found.
#' Then the "Dairy" leach rates for each cell in those areas are found. 
#' From these the requested summary statistic is calculated.
#' This function was prepared to calculate how water management sub-zone leach rates need to be adjusted if sheep and beef
#' are converted to Dairy land use.
#' @param LeachRates The raster of leach rates from which the average of the selected areas are to be calulcated
#' @param Predictors The raster stack of predictors used to determine the Dairy leach rates
#' @param ZoneLandUseLUC The raster of the combined water amanagement zone, landuse and land use capability. 
#' This raster has a raster attribute table which defines the class combinations.
#' @param ZoneLanduseLUCOfInterest A three column data frame of water management zones, land usees and 
#' land use capabilities to be summarised
#' @param MPITable The data frame look up table between the predictors and leach rate.
#' @return A single value

DairyLeachRateGeterer <- function(LeachRates = LeachRate,
                                  Predictors = PredictorRaster,
                             ZoneLandUseLUC = ZoneLanduseLUCRaster,
                             ZoneLanduseLUCOfInterest = c("Mana_8b","Sheep and/or Beef",3),
                             MPITable = MPILeachRateLookUpTable) {

  #Combine the criteria
  Criteria <- paste(ZoneLanduseLUCOfInterest,collapse="-")

  #Create a mask of the ZoneLandUseLUC raster for the area that matches the criteria
  RasterValue <- which(levels(ZoneLandUseLUC)[[1]]$category == Criteria)

    ZLLMask <- ZoneLandUseLUC == RasterValue
    ZLLMask[ZLLMask <= 0] <- NA
    #Get the predictor values for each cell
    PredictorsMask <- Predictors * ZLLMask

    PredictorValues <- values(PredictorsMask)
    
    #Only keep the cells with data in them
    PredictorValues <- PredictorValues[complete.cases(PredictorValues),]
    
    #Convert the data into a n x 5 matrix. This is needed as if only a single cell is returned, then it will be a vector, which won't convert to a data frame easily in the following step
    PredictorValues <- matrix(PredictorValues,ncol=5)
    
    #Convet to a dataframe and name it ready for comparison with the MPI lookup table
    PredictorValues <- as.data.frame(PredictorValues)
    names(PredictorValues) <- c("Climate","LU","PAW","Irri","Irgtd")
    
    #Convert the landuse to Dairy (i.e. a value of 2 in the MPI table)
    PredictorValues$LU <- 2

    #Look up the value, if they were "Dairy" and not irrigated
    DairyLeachRates <- merge(PredictorValues,MPITable[,c("Climate","LU","PAW","Irri","Irgtd","N_loss")],all.x = TRUE)
    
    #average them
    SummaryValue <- mean(DairyLeachRates$N_loss,na.rm=TRUE)

    return(SummaryValue)
}


#' Increase Dairy leach rates to WMZ values
#'
#' DairyLeachTOWMZRates prepares a leach rate raster based on an existing leach rate raster and increases all the Dairy
#' areas to the leach rates for their water management zone provided in a data frame
#' This function helps prepare the DairySector variant B leach rates, before adjustment for the consented and unconsented-known
#' @param LeachRateRaster A raster of the leach rates
#' @param ZLLRaster A raster of the zone, landue and land use capability combination codes. Same extent and resolution as CurrentLeachRateRaster
#' @param CombinedClassesDF A dataframe of Node names, loads, areas and leach rates (outpot from the Some-scenario-data-preparation) #' to be aggregated to water management zones and applied to the Dairy areas
#' @return A raster of leach rates
DairyLeachTOWMZRates <- function(LeachRateRaster = LeachRate, 
                                    ZLLRaster = ZoneLanduseLUCRaster,
                                   CombinedClassesDF = CombinedClasses) {
  #Parse the nodenames in the CombinedClases data frame to provide the water management zone
  CombinedClassesDF$SubZones <- sub('-.*','',CombinedClassesDF$CASMNodeNames)
  CombinedClassesDF$WMZ    <- sub("[a-z]$","",CombinedClassesDF$SubZones)
  
  #Aggregate loads and areas on water management zones
  WMZDairyRates <- aggregate(CombinedClassesDF[,c("Area_hectares","Load")]*!is.na(CombinedClassesDF$Load), 
                          by = list(CombinedClassesDF$WMZ), FUN = sum, na.rm=TRUE)
  names(WMZDairyRates)[1] <- "WMZ"
  
  #Calculate new leach rates
  WMZDairyRates$LeachRates <- WMZDairyRates$Load / WMZDairyRates$Area_hectares
   
  #Parse the category of the ZLL raster into its constituent parts
  SubZones    <- sub('-.*','',levels(ZLLRaster)[[1]]$category)
  Zones       <- sub("[a-z]$","",SubZones)    
  ZoneFactors <- as.factor(Zones)
  ZonesRaster <- reclassify(ZLLRaster,matrix(c(ZLLRaster@data@attributes[[1]]$ID,ZoneFactors),ncol=2))
  ZonesRaster <- ratify(ZonesRaster)
  rat <- levels(ZonesRaster)[[1]]
  rat$Zones <- levels(ZoneFactors)
  levels(ZonesRaster) <- rat

  Landuse                   <- sub('^[^-]*-(.*)-.*','\\1',levels(ZLLRaster)[[1]]$category)
  LanduseFactors <- as.factor(Landuse)
  LandUseRaster <- reclassify(ZLLRaster,matrix(c(ZLLRaster@data@attributes[[1]]$ID,LanduseFactors),ncol=2))
  LandUseRaster <- ratify(LandUseRaster)
  rat <- levels(LandUseRaster)[[1]]
  rat$Landuse <- levels(LanduseFactors)
  levels(LandUseRaster) <- rat

  #Create a Dairy Mask
  DairyMask <- LandUseRaster == which(levels(LandUseRaster)[[1]]$Landuse == "Dairy")
  #browser()
  #Check that the WMZ's in the WMZDairyRates exist in the raster, and remove them if they don't
  WMZDairyRates <- WMZDairyRates[WMZDairyRates$WMZ %in% unique(levels(ZonesRaster)[[1]]$Zones),]
  
  #Work through each WMZ and adjust the Dairy to the maximum of either the existing leach rate value or the leach rate provided
  
  #Initialise and output leach rate raster
  OutputLeachRate <- LeachRateRaster
  for (WMZ in WMZDairyRates$WMZ) {
    #browser()
    #Create a zone mask for the current zone
    ZoneMask <- ZonesRaster == which(levels(ZonesRaster)[[1]]$Zones == WMZ)
    
    AdjustedLeachRate <- DairyMask * ZoneMask * WMZDairyRates[WMZDairyRates$WMZ == WMZ,'LeachRates']
    OutputLeachRate <- max(OutputLeachRate,AdjustedLeachRate)
  }

  return(OutputLeachRate)
}


#*************************************************************************
#Function to reclassify leach rates of a specific number to another number
#This has been used to adjust the maximum leach rate of vegetable growing from 71 to a new value
#This was for scenario 13 A and B AKA Horticulture Mitigation A and B
LeachRateReclassifier <- function(LeachRates = LeachRate, OldValue = 71, NewValue = 46){
  OutLeachRates <- reclassify(LeachRates, rcl = matrix(c(OldValue,NewValue),ncol=2))
  return(OutLeachRates)
}
...
LoadComparer <- function(CASMFiles = c("CASMInputData1.xlsx","CASMInputData2.xlsx")){
  ...
  DiffuseData$load <- DiffuseData$`Land.Area.(ha)` * DiffuseData$`Export.Coeff.(kg/ha/yr)`
...
A large number of scenarios have been prepared using these data.
The scenarios are described [here](https://harrcons.sharepoint.com/:w:/r/sites/HorizonsPlanChange_2/Shared%20Documents/Reports/ScenarioSummary%2018%20August%202020.docx?d=wc6b1b8d41f28444bbd59f9d86f059b7b&csf=1&web=1&e=csq2rx).  
Each scenario has a slightly different method of being prepared.  
For added confusion, the naming of the scenarios has changed several times.
The following is an attempt to describe how to re-create each of the different scenarios.
It is a good idea to repeat a previous scenario and check output is identical before preparing variants.
In no particular order:  

### Horizons Scenario 0[a,b]

Naming system| Scenario number/name
-------------|-----------  
Horizons                 | 0[a,b] 
Streamlined Environmental| 2[a,b] 
Report                   | Operative Consented 
CASM input filename      | CASM-Inputs_SEL_Scenario_2[a,b]_IntensiveoperativeTable14_2Year[5,20]OrConsentedWithCoxCalibrated.xlsx

#### Scenario preparation process
1. Run the "Load Library" chunk
2. Run the "Some functions" chunk
3. Edit (then run) the File name setting code chunk so that the leach rate raster file name is set to MPI Leach rate tif file.  
4. Run the "load files" chunk.
5. Update the LeachRate raster using the ScenarioLeachRateRaster function with Intensive set to "TRUE", Landuse set to "NA", plan set to "Existing" and year set to [5,20]
6. Edit (then run) the "scenario preparation" chunk so that the "Scenario column of interest" is set to the appropriate column of the Horizons master spreadsheet. Note that this spreadsheet uses the Horizon's scenario numbering system i.e. scenario 3.  
7. Run the chunks to follow in turn until after the Diffuse Table chunk.  
8. DON'T run the "Dairy-expansion-scenario-preparation" chunk 
9. Run the "Adjust-Diffuse-table-for-sccenarios" chunk.  
10. Edit (then run) the Excel output chunk so that the output excel file has a sensible name. 
11. Check that the output file is reasonable by running the CASMScenarioInputChecker function.

***

### Horizons Scenario 1[a,b]

Naming system| Scenario number/name
-------------|-----------  
Horizons                 | 1[a,b] 
Streamlined Environmental| 3[a,b] 
Report                   | Proposed Consented 
CASM input filename      | CASM-Inputs_SEL_Scenario_3[a,b]_IntensiveTable14_2Year[5,20]OrConsentedWithCoxCalibrated.xlsx

#### Scenario preparation process
as for scenario 1 except in step 5 set plan to "PC2"

***

### Horizons Scenario 2[a,b,c,d]

Naming system| Scenario number/name
-------------|-----------  
Horizons                 | 2[a,b,c,d] 
Streamlined Environmental| 4[a,b,c,d] 
Report                   | Proposed Year 5 Pathway 
CASM input filename      | CASM-Inputs_SEL_Scenario_4[a,b,c,d]_IntensiveTable14_2Year50OrConsentedOrDH9WithCoxCalibrated.xlsx


#### Scenario preparation process
1. Run the "Load Library" chunk
2. Run the "Some functions" chunk
3. Edit (then run) the File name setting code chunk so that the leach rate raster file name is set to to MPI Leach rate tif file.  
4. Run the "load files" chunk.
5. Update the LeachRate raster using the ScenarioLeachRateRaster function with Intensive set to "TRUE", Landuse set to "NA", plan set to "PC2" and year set to 5
6. Edit (then run) the "scenario preparation" chunk so that the "Scenario column of interest" is set to the appropriate column of the Horizons master spreadsheet. Note that this spreadsheet uses the Horizon's scenario numbering system i.e. scenario 3.  
7. Run the chunks to follow in turn until after the Diffuse Table chunk.  
8. DON'T run the "Dairy-expansion-scenario-preparation" chunk 
9. Run the "Adjust-Diffuse-table-for-sccenarios" chunk.  
...
### Horizons Scenario 3[e,f,g,h]

Naming system| Scenario number/name
-------------|-----------  
Horizons                 | 3[e,f,g,h] 
Streamlined Environmental| 5[a,b,c,d] 
Report                   | Proposed Year 20 Pathway 
CASM input filename      | CASM-Inputs_SEL_Scenario_5a_IntensiveTable14_2Year20OrConsentedOrDH9WithCoxCalibrated.xlsx


#### Scenario preparation process
as above except in step 5 set year to 20

***

### Horizons Scenario 3i and 3j

Naming system| Scenario number/name
-------------|-----------  
Horizons                 | 3[i,j] 
Streamlined Environmental| N/A 
Report                   | Proposed Year-20 Pathway [E,F]
CASM input filename      | CASM-Inputs_Horizons_Scenario3[i,j]_IntensiveTable14_2Year20OrConsentedOrDH[9,18]AllDairyWithCoxCalibrated.xlsx

#### Scenario preparation process
as above except in step 5 set Landuse to "Dairy" and Mitigation set to 9 (for 3i) and 18 (for 3j)

***

### Horizons Scenario 3k and 3l

Naming system| Scenario number/name
-------------|-----------  
Horizons                 | 3[k,l] 
Streamlined Environmental| N/A 
Report                   | Proposed Year-20 Pathway [G,H]
CASM input filename      | CASM-Inputs_Horizons_Scenario3[k,l]_IntensiveTable14_2Year20OrConsentedOrDH[9,18]AllDairyVegLess36PctWithCoxCalibrated.xlsx

#### Scenario preparation process
1. Run the "Load Library" chunk
2. Run the "Some functions" chunk
3. Edit (then run) the File name setting code chunk so that the leach rate raster file name is set to the MPI Leach rate tif.  
4. Run the "load files" chunk.
5. Copy the LeachRate to a LeachRateCC
6. Create dairy-adjusted LeachRate rasters using the ScenarioLeachRateRaster function with leachrate set to LeachRateCC, Intensive set to "TRUE",  Landuse set to "Dairy" and mitigation set to 9 (for 3k) and 18 (for 3l)
7. Create sheep-and-beef-adjusted LeachRate rasters using the ScenarioLeachRateRaster function with leachrate set to LeachRateCC, Intensive set to "TRUE", Landuse set to "Sheep and/or Beef" and mitigation set to 9 (for 3k) and 18 (for 3l)
8. Create hort-veg-adjusted LeachRate rasters using the ScenarioLeachRateRaster function with leachrate set to LeachRateCC, Intensive set to "TRUE", Landuse set to "Horticulture-Veg" and mitigation set to 0.36
9. Create hort-other-adjusted LeachRate rasters using the ScenarioLeachRateRaster function with leachrate set to LeachRateCC, Intensive set to "TRUE",  Landuse set to "Horticulture-Other" and mitigation set to 0.36
10. Get the maximum of the hort-veg and hort-other leach rates using max(stack(leachrateveg,leachrateother))
11. Get the maximum of the veg and the dairy and sheep and beef leach rates for each of the mitigation variants, using the same method as in 9 above
12. Edit (then run) the "scenario preparation" chunk so that the "Scenario column of interest" is set to the appropriate column of the Horizons master spreadsheet. Note that this spreadsheet uses the Horizon's scenario numbering system i.e. scenario 3.
13. Run the chunks to follow in turn until after the Diffuse Table chunk.  
14. DON'T run the "Dairy-expansion-scenario-preparation" chunk 
15. Run the "Adjust-Diffuse-table-for-sccenarios" chunk.  
16. Edit (then run) the Excel output chunk so that the output excel file has a sensible name.
...
Horizons                 | 5 
Streamlined Environmental| N/A 
Report                   | Operative Expansion
CASM input filename      | CASM-Inputs_Horizons_Scenario5_11PctDairyExpanseOperativeTable14_2Year20WithCoxCalibrated.xlsx
...
3. Edit (then run) the File name setting code chunk so that the leach rate raster file name is set to the MPI Leach rate tif.  
4. Run the "load files" chunk.
5. Update the LeachRate raster using the ScenarioLeachRateRaster function with Intensive set to "TRUE", Landuse set to "NA", plan set to "Existing" and year set to 20
13. Run the chunks to follow in turn until after the Diffuse Table chunk.  
14. Run the "Dairy-expansion-scenario-preparation" chunk 
15. Don't run the "Adjust-Diffuse-table-for-sccenarios" chunk.  
16. Edit (then run) the Excel output chunk so that the output excel file has a sensible name.

***
### Horizons Scenario 8a

Naming system| Scenario number/name
-------------|-----------  
Horizons                 | 8 
Streamlined Environmental| N/A 
Report                   | Dairy Sector Pathway
CASM input filename      | CASM-Inputs_Horizons_Scenario8_IntensiveTable14_2Year20OrConsentedOrFedFarmWithCoxCalibrated.xlsx
...
3. Edit (then run) the File name setting code chunk so that the leach rate raster file name is set to the MPI Leach rate tif.  
4. Run the "load files" chunk.
5. Update the LeachRate raster using the ScenarioLeachRateRaster function with Intensive set to "TRUE", Landuse set to "NA", plan set to "PC2" and year set to 20
6. Edit (then run) the "scenario preparation" chunk so that the "Scenario column of interest" is set to the "Scenario.8.Fed.FarmersV2a" column of the Horizons master spreadsheet. Note that this spreadsheet uses the Horizon's scenario numbering system i.e. scenario 3.
13. Run the chunks to follow in turn until after the Diffuse Table chunk.  
14. DON'T run the "Dairy-expansion-scenario-preparation" chunk 
15. Run the "Adjust-Diffuse-table-for-sccenarios" chunk.  
16. Edit (then run) the Excel output chunk so that the output excel file has a sensible name.
...
Streamlined Environmental| N/A 
Report                   | Dairy Sector Pathway
CASM input filename      | CASM-Inputs_Horizons_Scenario8b_IntensiveTable14_2Year20OrConsentedOrFedFarmGeneralisedWithCoxCalibrated.xlsx
...
4. Run the "load files" chunk.
5. Update the LeachRate raster using the ScenarioLeachRateRaster function with Intensive set to "TRUE", Landuse set to "NA", plan set to "PC2" and year set to 20
6. Edit (then run) the "scenario preparation" chunk so that the "Scenario column of interest" is set to the "Scenario.8.Fed.FarmersV2b" column of the Horizons master spreadsheet. Note that this spreadsheet uses the Horizon's scenario numbering system i.e. scenario 3. These data are needed to adjust the leach rate raster so all dairy areas in water management zones that have known unconsented Overseer data, have their Dairy leach rates match the adjustment that the unconsented known dairy farms would get.
7. Run the "DairyLeachTOWMZRates" function to create a new leach rate raster
8. Edit (then run) the "scenario preparation" chunk so that the "Scenario column of interest" is set to the "Scenario.8.Fed.FarmersV2a" column of the Horizons master spreadsheet. Note that this spreadsheet uses the Horizon's scenario numbering system i.e. scenario 3. This is required to adjust the consented and unconsented with Overseer data properties.
13. Run the chunks to follow in turn until after the Diffuse Table chunk.  
14. DON'T run the "Dairy-expansion-scenario-preparation" chunk 
15. Run the "Adjust-Diffuse-table-for-sccenarios" chunk.  
16. Edit (then run) the Excel output chunk so that the output excel file has a sensible name.
...
### Horizons Scenario 12a, 12b

Naming system| Scenario number/name
-------------|-----------  
Horizons                 | 12[a,b]
Streamlined Environmental| N/A
Report                   | Post-regulation[A,B]  
CASM input filename      | CASM-Inputs_Horizons_Scenario12[A,B]_{2017PointSources_]ConsentedOrBaseOrCoxCalibrated.xlsx

#### Scenario preparation process
1. Run the "Load Library" chunk
2. Run the "Some functions" chunk
3. Edit the "Set-directories-and-filenames" chunk so that the "PointSourceYear" variable matches the scenario variant (A=2012, B= 2017)
4. Edit (then run) "Set-directories-and-filenames" chunk so that the leach rate raster file name is set to the MPI Leach rate tif.  
4. Run the "load files" chunk.
6. Edit (then run) the "scenario preparation" chunk so that the "Scenario column of interest" is set to the appropriate column of the Horizons master spreadsheet. Note that this spreadsheet uses the Horizon's scenario numbering system i.e. scenario 3. 
7. Run the chunks to follow in turn until after the Diffuse Table chunk.  
8. DON'T run the "Dairy-expansion-scenario-preparation" chunk 
...
### Horizons Scenario 13

Naming system| Scenario number/name
-------------|-----------  
Horizons                 | 13 
Streamlined Environmental| 13 
Report                   | Horticulture Mitigation 
CASM input filename      | CASM-Inputs_Horizons_Scenario13_HorticultureInSWestTo46OrCoxCalibrated.xlsx
...
3. Edit (then run) the File name setting code chunk so that the leach rate raster file name is set to the MPI Leach rate tif.  
4. Run the "load files" chunk.
5. Update the LeachRate raster using the LeachRateReclassifier function. 71 is unique to south west climate vegetables, so change this to whatever is required.
6. DON'T run the "scenario preparation" chunk  
7. Run the chunks to follow in turn until after the Diffuse Table chunk.  
8. DON'T run the "Dairy-expansion-scenario-preparation" chunk 
9. DON'T RUN the "Adjust-Diffuse-table-for-sccenarios" chunk.  
10. Edit (then run) the Excel output chunk so that the output excel file has a sensible name. 
11. Check that the output file is reasonable by running the CASMScenarioInputChecker function.
...
#For Horizons scenario: 3, Streamlined Scenario: 5, Report scenario: Proposed Year 20 Pathway
...
ZoneLanduseLUCRasterFile           <- file.path(GISDataDirectory,"ZoneLanduseLUCRaster 250 x 250.tif")
...
HorizonsConsentedLeachRatesFile    <- file.path(DataDirectory,
                                                "20200417_Master dairy farm data_Working Document – For Purposes of Scenario ModellingV5.xlsx")
...
PointSourceYear                    <- "2012" #2012 is the default and has been used for everything except for one scenario, The other option is "2017"

#For the Native land use scenario need to load modified files
#SubZoneLanduseLUCShapeFile         <- file.path(GISDataDirectory,"SubZoneNativeLanduseLUC","SubZoneNativeLanduseLUC")
#LeachRateRasterFile                <- file.path(GISDataDirectory,"MPINativeLeachRateRaster 250x250.tif")
#ZoneLanduseLUCRasterFile           <- file.path(GISDataDirectory,"ZoneNativeLanduseLUCRaster 250 x 250.tif")

#If a scenarios leachrate raster needs to be re-created then this can be don 
...
PredictorRaster <- stack(PredictorRasterFile)
...
PointSourceSites <- plyr::rename(PointSourceSites,replace=c("Annual.load.kg/yr" = "TN.kgpy"))
...
#                                   LanduseData = SubZoneLanduseLUCShapeFile,
...
#writeRaster(LeachRate,file.path(GISDataDirectory,"MPINativeLeachRateRaster 250x250.tif"),overwrite=TRUE)
#******************************************************************
...
# # Or to recreate leach rate rasters from the Cox-calibrated leach rates adjusted for Table 14.2 use this (this example does PC2 year 20 for IFLU, no landuse mitigation):
# LeachRateTable14.2 <- ScenarioLeachRateRaster(CurrentLeachRateRaster = LeachRate,
#                                            LUC_To_Leach_Table = LeachRateLimits,
#                                     ZLLRaster = ZoneLanduseLUCRaster,
#                                     ZoneLanduseLUC_LUTFile = ZoneLanduseLUCCode_To_ClassLUTFile,
#                                     plan = "PC2",
...
#                                     Mitigation = 9,
#                                     MPIPredictorRasterFile = PredictorRasterFile)
# # Or to adjust for mitigation of  a certain land use, e.g. Scenario 3i. Note that this requires the CurrentLeachRateRaster to be the Cox-calibrated version
# LeachRateTable14.2AndMitigatedAbsolute <- ScenarioLeachRateRaster(CurrentLeachRateRaster = LeachRate,
#                                            LUC_To_Leach_Table = LeachRateLimits,
#                                     ZLLRaster = ZoneLanduseLUCRaster,
#                                     ZoneLanduseLUC_LUTFile = ZoneLanduseLUCCode_To_ClassLUTFile,
#                                     plan = "PC2",
...
#                                     Mitigation = 9,
#                                     MPIPredictorRasterFile = PredictorRasterFile)
# LeachRateTable14.2AndMitigatedRelative <- ScenarioLeachRateRaster(CurrentLeachRateRaster = LeachRate,
#                                            LUC_To_Leach_Table = LeachRateLimits,
#                                     ZLLRaster = ZoneLanduseLUCRaster,
#                                     ZoneLanduseLUC_LUTFile = ZoneLanduseLUCCode_To_ClassLUTFile,
#                                     plan = "PC2",
#                                     year = 20,
#                                     Intensive = TRUE,
#                                     Landuse = "Horticulture-Veg",
#                                     Mitigation = 0.36,
#                                     MPIPredictorRasterFile = PredictorRasterFile)
#For multiple conditions it is necesary to create separate leachrate rasters, then combine them using max(stack(LeachRate1,LeachRate2...)).
#Note also that there are two types of Horticulture in the Landuse raster (Horticulture-Veg and Horticulture-Other), which map to the single "Horticulture" class in the MPI lookup table, so both Horticulture's need to be done.This assumes leachrates are lifted up to the table level if they were previously below.
#Do to do Dairy and "Sheep and Beef" and Horticulture it is necesary to create 4 different leachrate rasters, all starting from the CoxCalibrated version. 
```
...
This location information needs to be used to update some of these sites' attributes. 
```{r Some-scenario-data-preparation}
...
#New version, as what was really wanted was the maximum of the Table value or the discretonary pathway via the 90th/75th Ptile calculation
#Create a new column that meets the Federated Farmers criteria of the lesser of consented, or the maximum of PC2 year 20 or the the lesser of the 90 % of the overseer base values, or the 75th percentile of the Overseeer base vaues for the management zone
FedFarmValuesV2 <- apply(HorizonsPropertyData,1, function(x) {
  WaterManagementZone <- x["Water.Management.Zone"]
  #browser()
  PC2Year20 <- as.numeric(x["Yr.20.new.table"])
  WMZBaselines <- HorizonsPropertyData$`Base.N.Leaching.(kg.N/ha/yr)`[HorizonsPropertyData$Water.Management.Zone == WaterManagementZone]
  Percentile75th <- quantile(WMZBaselines,probs = 0.75,names=FALSE,na.rm=TRUE)
  NewLossRate <- if(x["Consented"] == "Y") as.numeric(x["Target.N.Leaching.(kg.N/ha/yr)"]) else max(PC2Year20,min(Percentile75th,0.9 * as.numeric(x["Base"]),na.rm=TRUE),na.rm=TRUE)
})

#This is a variant of the Dairy sector calculation to enable idetification of leach rates for known-unconsented farms. This gets used to adjust unknown unconsented farms.
#Create a new column that partially meets the Federated Farmers criteria for just the unconsented properties of the lesser of the 90 % of the overseer base values, or the 75th percentile of the Overseeer base vaues for the management zone
FedFarmValuesV2b <- apply(HorizonsPropertyData,1, function(x) {
  WaterManagementZone <- x["Water.Management.Zone"]
  PC2Year20 <- x["Yr.20.new.table"]
  WMZBaselines <- HorizonsPropertyData$`Base.N.Leaching.(kg.N/ha/yr)`[HorizonsPropertyData$Water.Management.Zone == WaterManagementZone]
  Percentile75th <- quantile(WMZBaselines,probs = 0.75,names=FALSE,na.rm=TRUE)
  NewLossRate <- if(x["Consented"] == "Y") NA else min(Percentile75th,0.9 * as.numeric(x["Base"]),na.rm=TRUE)
})

HorizonsPropertyData <- HorizonsPropertyData[,c(3,15,23:30,47:60)]

#Add on the FedFarmers Scenario
HorizonsPropertyData$Scenario.8.Fed.Farmers <- FedFarmValues
HorizonsPropertyData$Scenario.8V2a.Fed.Farmers <- FedFarmValuesV2
HorizonsPropertyData$Scenario.8V2b.Fed.Farmers <- FedFarmValuesV2b
...
ScenarioColumnOfInterest <- "Scenario.8V2a.Fed.Farmers"
#ScenarioColumnOfInterest <- "Scenario.8V2b.Fed.Farmers"
#ScenarioColumnOfInterest <- "Scenario.10.-.LesserOfConsentedorPC2Table14.2"
#ScenarioColumnOfInterest <- "Scenario.12.-.ConsentedorBase.N.Leaching"

#Note that data_long is a tibble, not a data frame which confuses me.
data_long <- as.data.frame(data_long)

data_long$Load <- data_long$"hectares" * data_long[,ScenarioColumnOfInterest]
#Remove any NA values. For the base N leaching scenario there were two properties without base N leaching data.
#For the FedFarmersV2b version the consented farms were set to NA to prevent their inclusion in the area and so get a correct estimate of leach rates for just the known unconsented farms (which is the value that is needed to apply to the unknown unconsented dairy farms)
data_long <- data_long[!is.na(data_long$Load),]
...
```{r Analyse-management-sub-zones}
...
#writeOGR(SubZoneLanduseLUCSpatial,dsn =dirname(SubZoneLanduseLUCShapeFile) ,layer=basename(SubZoneLanduseLUCShapeFile),driver='ESRI Shapefile',overwrite_layer=TRUE)
...
```{r Create-CASM-point-source-table}
...
```{r Create-CASM-measurement-site-table}
...
```{r Create-CASM-diffuse-inputs-table}
...
```{r Dairy-expansion-scenario-preparation}
...
DairyConversionData$NewLandArea <- NA
...
for (WaterManagementSubZone in DairyAreas$Zone_Code){
  ...
  #if(WaterManagementSubZone == "West_8") browser()
  ...
  for (LUC in LUCClasses) {
     #if(WaterManagementSubZone == "Whai_4d") browser()
    ...
    #Check that a Dairy node exists for the current LUC. If it doesn't create a node with no land area ready for conversion
    if (nrow(DairyNodeOfInterest) < 1) {
      DairyNodeOfInterest <- SAndBNodeOfInterest
      DairyNodeOfInterest$Landuse                   <- "Dairy"
      DairyNodeOfInterest$`Node Name`     <- sub("-.*-","-Dairy-",DairyNodeOfInterest$`Node Name`)
      DairyNodeOfInterest$`Land Area (ha)`          <- 0
      DairyNodeOfInterest$`Discharge Location (km)` <- DairyNodeOfInterest$`Discharge Location (km)` + 0.005 #Need to adjust the location so that it is offset from all the other sub nodes.
      #DairyNodeOfInterest$`Export Coeff (kg/ha/yr)` <- 0
      #browser()
      DairyNodeOfInterest$`Export Coeff (kg/ha/yr)` <- DairyLeachRateGeterer(ZoneLanduseLUCOfInterest = data.frame(WaterManagementSubZone,"Sheep and/or Beef",LUC))
      DairyConversionData <- rbind(DairyConversionData,DairyNodeOfInterest)
    }
...
DiffuseInputsSiteExtendedTable <- merge(DiffuseInputsSiteExtendedTable, DairyConversionUpdateData[,c("Node Name","NewLandArea")], by = "Node Name", all.x = TRUE)

#Need to add any extra created nodes
ExtraNodeIndices <- which(!DairyConversionUpdateData$`Node Name` %in% DiffuseInputsSiteExtendedTable$`Node Name`)
DiffuseInputsSiteExtendedTable <- rbind(DiffuseInputsSiteExtendedTable,DairyConversionUpdateData[ExtraNodeIndices,])

DiffuseInputsSiteExtendedTable$`Land Area (ha)` <- ifelse(is.na(DiffuseInputsSiteExtendedTable$NewLandArea),DiffuseInputsSiteExtendedTable$`Land Area (ha)`,DiffuseInputsSiteExtendedTable$NewLandArea)
DiffuseInputsSiteExtendedTable <- subset(DiffuseInputsSiteExtendedTable, select=-c(NewLandArea))
```
...
```{r Adjust-Diffuse-table-for-scenarios}
CombinedClasses$TotalNodeArea <- 
  DiffuseInputsSiteExtendedTable$'Land Area (ha)'[match(CombinedClasses$CASMNodeNames,
                                                        DiffuseInputsSiteExtendedTable$'Node Name')]
CombinedClasses$NodeAveLeachRate <- 
  DiffuseInputsSiteExtendedTable$"Export Coeff (kg/ha/yr)"[match(CombinedClasses$CASMNodeNames,
                                                                 DiffuseInputsSiteExtendedTable$'Node Name')]
...
```{r Create-CASM-input-spreadsheet}
...
saveWorkbook(Out, file.path(DataDirectory,"CASM-Inputs_Horizons_Scenario8b_IntensiveTable14_2Year20OrConsentedOrFedFarmGeneralisedWithCoxCalibrated.xlsx"), overwrite = T)
```
