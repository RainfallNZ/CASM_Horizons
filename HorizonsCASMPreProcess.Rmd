...
#Note that the following is the filename on Tims Computer for the MPIPredictor raster brick
MPIPredictorRasterFileName <- "PredictorRaster 250x250.tif"

ScenarioLeachRateRaster <- function(CurrentLeachRateRaster = LeachRate, LUC_To_Leach_Table = LeachRateLimits, ZoneLanduseLUCRasterFile = ZoneLanduseLUCRasterFileName, ZoneLanduseLUCCode_To_ClassLUTFile = ZoneLanduseLUCCode_To_ClassLUTFileName,
                                     plan = "PC2", year = 20,
                                    Intensive = FALSE,
                                    MPIPredictorRasterFile = MPIPredictorRasterFileName) {
...
LeachRate <- stack(file.path(GISDataDirectory,LeachRateRasterFileName))[[1]]
...
ZoneLanduseLUCRaster <- raster(file.path(GISDataDirectory,ZoneLanduseLUCRasterFileName), RAT=TRUE)
...
saveWorkbook(Out, file.path(DataDirectory,"CASM-Inputs_IntenseLanduseTable14-2_20200513.xlsx"), overwrite = T)
```
