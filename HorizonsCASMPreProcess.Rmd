...
ScenarioLeachRateRaster <- function(CurrentLeachRateRaster = LeachRate, LUC_To_Leach_Table = LeachRateLimits, ZLLRaster = ZoneLanduseLUCRaster, ZoneLanduseLUC_LUTFile = ZoneLanduseLUCCode_To_ClassLUTFile,
  ...
  IrrigatedRaster <- stack(file.path(GISDataDirectory,MPIPredictorRasterFile))[[5]]
...
#Function to find the total area of each Climate-Plant Available Water class within the "target" Dairy-Land Use Capability areas, and all the irrigated aheep and beef - land use capability combinations.

TargetZoneAreaFinder <- function(GISDirectory=GISDirectory,
                                 ClimateShapeFile=ClimateShapeFile,
...
ClimateSpatial <- readOGR(file.path(GISDataDirectory,ClimateShapeFile),ClimateShapeFile, stringsAsFactors = FALSE)
...
SubZoneLanduseLUCSpatial <- readOGR(file.path(GISDataDirectory,SubZoneLanduseLUCShapeFile),SubZoneLanduseLUCShapeFile,stringsAsFactors = FALSE)
SubZoneLanduseLUCSpatial <- spTransform(SubZoneLanduseLUCSpatial,CRS("+init=epsg:2193") )

#Load irrigated land spatial data
IrrigatedLandSpatial <- readOGR(file.path(GISDataDirectory,IrrigatedShapeFile),IrrigatedShapeFile,stringsAsFactors = FALSE)
...
PAWSpatial <- readOGR(file.path(GISDataDirectory,PAWShapeFile),PAWShapeFile, stringsAsFactors = FALSE)
...
TargetZones <- readOGR(file.path(GISDataDirectory,TargetSubZonesFile),TargetSubZonesFile, stringsAsFactors = FALSE)
...
MPILeachRateRaster <- stack(file.path(GISDataDirectory,LeachRateRasterFile))
#Get the MPI predictor data
MPIPredictorRaster <- stack(file.path(GISDataDirectory,PredictorRasterFile))

  #Load MPI lookup table of total nitrogen leaching rates based on climate, land use, PAW, irrigation, irrigable land
  MPILeachRateLookUpTable<- read.csv(file.path(DataDirectory,MPILeachRateLUTFile))
  ...
  if(!exists("MPILeachRateLookUpTable")){ MPILeachRateLookUpTable <- read.csv(file.path(DataDirectory,MPILeachRateLUTFile))}
...
write.table(InfoOfInterest, file.path(DataDirectory,"MPIClassAreas.csv"),sep=",",row.names = FALSE,quote=FALSE)
 ...
 # The last Check is to find the percent that they have changed by.
...
CASMScenarioInputChecker <- function(CASMCalibratedFile="D:\\Projects\\LWP\\Horizons\\Data\\CASM-Inputs_CoxCalLeach.xlsx", CASMFileToBeChecked="D:\\Projects\\LWP\\Horizons\\Data\\CASM-Inputs_FullTable14.2.xlsx") {
  ...
  ChangeResults = list(Sheet1To3Same = Sheet1To3Same,DiffuseNodeDetailsTheSame = DiffuseNamesStreamsAndAreaSame,ChangeTable[IndicesOfChange,])
...
NitrogenLoadsFile <- "N_ExpCoeff_Est_HZ_Feb20.rdata"
...
SubZonesFile                       <- "Water_Management_Subzones_cleaned"
LanduseLUCShapeFile                <- "Landuse_LUC Original"
SubZoneLanduseLUCShapeFile         <- "ZoneLanduseLUC"                   #May be created if necesary
IrrigatedShapeFile                 <- "irrigated-land-area-2017-Manawatu"
PAWShapeFile                       <- "FSL_Horizons"
ClimateShapeFile                   <- "ClimateRegions-Horizon"
CASMModelDomainFile                <- "CASMModelDomains"
TargetSubZonesFile                 <- "Target_subzones"
MeasurementSitesFile               <- "MeasurementSites"                 #An output filename
PointSourceSitesFile               <- "PointSourceSites"                 #An output filename
WaterManagementZonesForMappingFile <- "WaterManagementZonesForMapping"   #An output filename
CASMStreamNetworkFile              <- "CASM-StreamNetwork"

#Set the various raster file names
PredictorRasterFile                <- "PredictorRaster 250x250.tif"
#Baseline MPI leach rates
#LeachRateRasterFile                <- "MPILeachRateRaster 250x250.tif"
#Various scenario leach rates these have already been corrected for the Tim Cox leach rate calibration corrections
#LeachRateRasterFile                 <- "ScenarioLeachRasters_IntensiveLanduseTable14-2.tif"
#LeachRateRasterFile                <- "ScenarioLeachRastersFullTable14-2.tif"
#LeachRateRasterFile                <- "ScenarioLeachRasters_IntensiveLanduseTable14_2_Year5.tif"
#LeachRateRasterFile                 <- "ScenarioLeachRasters_IntensiveLanduseExistingTable14_2_Year20.tif"
LeachRateRasterFile                 <- "ScenarioLeachRasters_IntensiveLanduseExistingTable14_2_Year5.tif"
...
HorizonsConsentedLeachRatesFile    <- "20200417_Master dairy farm data_Working Document – For Purposes of Scenario ModellingV3.xlsx"
...
RECReachNetwork <- readOGR(dsn =file.path(GISDataDirectory,REC_rivers),layer = REC_rivers)
...
CASMModelDomains <- readOGR(file.path(GISDataDirectory,CASMModelDomainFile),layer=CASMModelDomainFile, stringsAsFactors = FALSE)
#Explicitly set projection to NZTM
...
load(file.path(DataDirectory,NitrogenLoadsFile))
...
writeOGR(MeasurementSitesSpatial,dsn = file.path(GISDataDirectory,MeasurementSitesFile),MeasurementSitesFile,driver='ESRI Shapefile',overwrite_layer = TRUE)
...
#Save a copy for external use
writeOGR(PointSourceSitesSpatial,dsn = file.path(GISDataDirectory,PointSourceSitesFile),PointSourceSitesFile,driver='ESRI Shapefile',overwrite_layer = TRUE)

#Load the subzone polygons
SubZonePolygons <- readOGR(dsn =file.path(GISDataDirectory,SubZonesFile),layer = SubZonesFile)
...
# 
# #Save a copy for external use
# writeOGR(SubZonePolygons,dsn = file.path(GISDataDirectory,WaterManagementZonesForMappingFile),WaterManagementZonesForMappingFile,driver='ESRI Shapefile',overwrite_layer = FALSE)
...
#Load the spatial data with the landuse, land use capability and submanagement zones alltogether
#LanduseLUCSpatial <- readOGR(dsn =file.path(GISDataDirectory,LanduseLUCShapeFile),layer = LanduseLUCShapeFile)
#LanduseLUCSpatial <- spTransform(LanduseLUCSpatial,CRS("+init=epsg:2193") )
SubZoneLanduseLUCSpatial <- readOGR(file.path(GISDataDirectory,SubZoneLanduseLUCShapeFile),layer=SubZoneLanduseLUCShapeFile)
SubZoneLanduseLUCSpatial <- spTransform(SubZoneLanduseLUCSpatial,CRS("+init=epsg:2193") )

#Load the leachingrate data if it exists
LeachRate <- stack(file.path(GISDataDirectory,LeachRateRasterFile))[[1]]
...
# writeRaster(LeachRate,file.path(GISDataDirectory,LeachRateRasterFile),overwrite=TRUE)
#******************************************************************

#Load the ZoneLanduseLUC raster if it exists
ZoneLanduseLUCRaster <- raster(file.path(GISDataDirectory,ZoneLanduseLUCRasterFile), RAT=TRUE)
...
if (basename(LeachRateRasterFile) == "MPILeachRateRaster 250x250.tif") {
  ...
  #Background
  #When Tim Cox was calibrating his model he found that the attenuation rates that he needed to get catchment nutrient balance were smaller than what you would expect. So he decreased the leach rates to make the attenuation rates mode believeable.
...
HorizonsPropertyData <- read.xlsx(file.path(DataDirectory,HorizonsConsentedLeachRatesFile),sheet = "Dairy farms - target catchments", startRow = 3)
...
HorizonsPropertyData <- HorizonsPropertyData[,c(3,23:30,47:58)]
...
#there are some properties that are in multiple sub-management zones. Find them,divide them and share the LUC areas amongst the divided parts, adding the extra divided parts to the end of the master table
{
  ...
  #Work through each of these mulit-zone properties and divide them into single sub management zones rows
...
#writeOGR(SubZoneLanduseLUCSpatial,file.path(GISDataDirectory,SubZoneLanduseLUCShapeFile),layer=SubZoneLanduseLUCShapeFile,driver='ESRI Shapefile')
...
writeOGR(CompleteSpatialNetwork, file.path(GISDataDirectory,CASMStreamNetworkFile),layer= CASMStreamNetworkFile, driver="ESRI Shapefile",overwrite_layer=TRUE)
...
CombinedClasses$NewRate <- with(CombinedClasses, (LeachRates * Area_hectares + NodeAveLeachRate * pmax(0,AreaDiff)) / TotalNodeArea)
...
saveWorkbook(Out, file.path(DataDirectory,"CASM-Inputs_SEL_Scenario_2a_IntensiveOperativeTable14_2Year5OrConsentedWithCoxCalibrated.xlsx"), overwrite = T)
```
